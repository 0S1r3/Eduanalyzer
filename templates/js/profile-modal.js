...
// Изменение изображения при нажатии на фото профиля
document.addEventListener('DOMContentLoaded', () => {
    const userPhoto = document.getElementById('userPhotoMain');
    const modal = document.getElementById('changePhotoModal');
    const closeModal = document.getElementById('closeChangePhotoModal');
    const form = document.getElementById('changePhotoForm');
    const flashMessages = document.getElementById('newFlashMessages');

    // Открыть модальное окно при клике на фото
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            // Создаем новый контейнер для flash-сообщений
            const newFlashContainer = document.createElement('div');
            newFlashContainer.id = 'newFlashMessages';
            modal.querySelector('.header_upload').insertAdjacentElement('afterend', newFlashContainer); // Добавляем в начало модального окна
